...
{	
 	...
 	else if(dec_color.B[dec_color.q] > 0) 
	{
 	...
 	dec_color.extended_data[dec_color.x.y][dec_color.x.x] = (unsigned char)dec_color.Rx;
}
	...
	for (;;)
	{
		...
		{			
     		if((dec_color.w - dec_color.x.x) + 1 >= (1 << dec_color.J[dec_color.RUNindex]))
			{
        		...
        		{
        			dec_color.extended_data[dec_color.x.y][dec_color.x.x + i] = (unsigned char)dec_color.Ra;	
				}
    	 	...
    	 	else
			{
            	...
            	{
            		dec_color.extended_data[dec_color.x.y][i] = (unsigned char)dec_color.Ra;
				}			
