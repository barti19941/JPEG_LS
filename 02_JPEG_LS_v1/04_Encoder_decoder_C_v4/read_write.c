...
/**********************************************************************
	WritePpmPic	:	This function save data to *.ppm file.
						data - pointer to data to save
						w - width of image
						h - height of image
**********************************************************************/
void WritePpmPic(char*** data, int w, int h)
{
	...
	for (i = 0; i < h; i++)
	{
		for (j = 0; j < w; j++)
		{
			...
			{
				itoa((unsigned char)data[k][i][j], buff, 10);
				if ((unsigned char)data[k][i][j] >= 100)
				{
					buff[3] = ' ';
					fwrite(buff, sizeof(char), 4, outfile);
				}
				else if ((unsigned char)data[k][i][j] >= 10)
				{
					buff[2] = ' ';
					fwrite(buff, sizeof(char), 3, outfile);
				}
...
/**********************************************************************
	WriteJlsColorSampleIlvPic	:	This function save color encoded data 
...
/**********************************************************************
	WriteJlsColorNoIlvPic	:	This function save color encoded data 
...
/**********************************************************************
	ReadJlsPicColorLineIlv	:	This function read raw_data from *.jls
						color line ilv file.
						w - width of image
						h - height of image
						size - lenght of data bitstream						
...
/**********************************************************************
	ReadJlsPicGray	:	This function read raw_data from *.jls gray
						file.
						w - width of image
						h - height of image
						size - lenght of data bitstream						
