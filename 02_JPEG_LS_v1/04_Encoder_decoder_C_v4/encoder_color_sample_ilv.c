...
static void RunModeProcessing()
{
 	...
 	while(enc_color.RUNcnt >= (1 << enc_color.J[enc_color.RUNindex]))
	{
 	...
 	if((enc_color.RUNval[0] != enc_color.Ix[0]) || (enc_color.RUNval[1] != enc_color.Ix[1]) || (enc_color.RUNval[2] != enc_color.Ix[2]))
	{
	...
	if(enc_color.Ra[0] != enc_color.Ix[0] || enc_color.Ra[1] != enc_color.Ix[1] || enc_color.Ra[2] != enc_color.Ix[2] )
	{
		for (i = 0; i < COMPONENTS_COUNT; i++)
		{
			...
			{
				enc_color.Errval = enc_color.Ix[i] - enc_color.Ra[i];	
			}    
	  		else
	  		{
			...
			if((enc_color.RItype == 0) && (enc_color.Ra[i] > enc_color.Rb[i]))
			{
...
static int ContextModeling()
{
	...
	for (i = 0; i < COMPONENTS_COUNT; i++)
	{
		enc_color.D[i][0] = enc_color.Rd[i] - enc_color.Rb[i];
    	enc_color.D[i][1] = enc_color.Rb[i] - enc_color.Rc[i];
    	enc_color.D[i][2] = enc_color.Rc[i] - enc_color.Ra[i];
	}
    /* mode selection */
