...
static encoder_color_no_ilv enc_color;
...
static void GetNextSample()
{
	...
	enc_color.Ix = (unsigned char)enc_color.extended_data[enc_color.x.y][enc_color.x.x];	
	enc_color.Ra = (unsigned char)enc_color.extended_data[enc_color.x.y][enc_color.x.x - 1];
	enc_color.Rb = (unsigned char)enc_color.extended_data[enc_color.x.y - 1][enc_color.x.x];
	enc_color.Rc = (unsigned char)enc_color.extended_data[enc_color.x.y - 1][enc_color.x.x - 1];
	enc_color.Rd = (unsigned char)enc_color.extended_data[enc_color.x.y - 1][enc_color.x.x + 1];
...
static void Initializations(char** data, int w, int h)
{
	int i;
	/* clear bitstream */
	enc_color.bitstream = Allocate1D(2 * w * h);
...
/**********************************************************************
	EncodeColorNoIlv	:	This function encode color 3D raw data into jpegls
						comporessed data.
...
char** EncodeColorNoIlv(char*** data, int w, int h, int* size)
{
	...
	for (i = 0; i < COMPONENTS_COUNT; i++)
	{
		...
		dout[i] = Allocate1D(byte_count);
		/* rewrite bitstream */
		for (j = 0; j < byte_count; j++)
		{
			dout[i][j] = enc_color.bitstream[j];	
		}
		*(size+i) = byte_count;    	
		Free1D(enc_color.bitstream);
		Free2D(enc_color.extended_data);
