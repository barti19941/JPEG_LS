...
{	
 	...
 	else if(dec_color.B[dec_color.q] > 0) 
	{
 	...
 	dec_color.extended_data[cmp][dec_color.x.y][dec_color.x.x] = dec_color.Rx;
}
...
static void RunModeProcessing()
{
	...
	for (;;)
	{
		...
		{			
     		if((dec_color.w - dec_color.x.x) + 1 >= (1 << dec_color.J[dec_color.RUNindex]))
			{
        		...
        		{
        			dec_color.extended_data[0][dec_color.x.y][dec_color.x.x + i] = dec_color.Ra[0];	
        			dec_color.extended_data[1][dec_color.x.y][dec_color.x.x + i] = dec_color.Ra[1];	
        			dec_color.extended_data[2][dec_color.x.y][dec_color.x.x + i] = dec_color.Ra[2];	
				}
    	 	...
    	 	else
			{
            	...
            	{
            		dec_color.extended_data[0][dec_color.x.y][i] = dec_color.Ra[0];
            		dec_color.extended_data[1][dec_color.x.y][i] = dec_color.Ra[1];
            		dec_color.extended_data[2][dec_color.x.y][i] = dec_color.Ra[2];
				}		
	...
	if (run_flag)
	{
		...
		for (i = 0; i < COMPONENTS_COUNT; i++)
		{
			...
			dec_color.extended_data[i][dec_color.x.y][dec_color.x.x] = dec_color.Rx;		
