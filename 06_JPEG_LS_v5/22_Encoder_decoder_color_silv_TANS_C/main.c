...
int main()
{
	...
	if (CODING_MODE == 1)
	{		
		printf("Coding variant: tANS + tANS.\n");
		printf("Coding mode: regular + run.\n");
		if (PIXELS_MODE == 1)
		{
			printf("Pixels mode: modified.\n");
			f = fopen("raport_color_silv_tans_tans_regular_run_modified.txt", "wt");
		}
		else
		{
			printf("Pixels mode: no-modified.\n");
			f = fopen("raport_color_silv_tans_tans_regular_run_nomodified.txt", "wt");
		}
	}
	else
	{
		printf("Ccoding variant: tANS.\n");
