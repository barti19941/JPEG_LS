...
int main()
{
	...
	if (CODING_MODE == 1)
	{		
		printf("Coding variant: rANS + rANS.\n");
		printf("Coding mode: regular + run.\n");
		if (PIXELS_MODE == 1)
		{
			printf("Pixels mode: modified.\n");
			f = fopen("raport_color_lilv_rans_rans_regular_run_modified.txt", "wt");
		}
		else
		{
			printf("Pixels mode: no-modified.\n");
			f = fopen("raport_color_lilv_rans_rans_regular_run_nomodified.txt", "wt");
		}
	}
	else
	{
		...
		{
			printf("Pixels mode: modified.\n");
			f = fopen("raport_color_lilv_rans_regular_modified.txt", "wt");
		}
		else
		{
			printf("Pixels mode: no-modified.\n");
			f = fopen("raport_color_lilv_rans_regular_nomodified.txt", "wt");
		}
	}
	for (i = t[0]; i <= t[1]; i++)
	{
