...
int main()
{
	...
	{		
		...
		if (PIXELS_MODE == 1)
		{
			...
			f = fopen("raport_color_silv_rans_golomb_rice_regular_run_modified.txt", "wt");
		}
		else
		{
			printf("Pixels mode: no-modified.\n");
			f = fopen("raport_color_silv_rans_golomb_rice_regular_run_nomodified.txt", "wt");
		}
	}
	else
	{
		...
		{
			printf("Pixels mode: modified.\n");
			f = fopen("raport_color_silv_rans_regular_modified.txt", "wt");
		}
		else
		{
			printf("Pixels mode: no-modified.\n");
			f = fopen("raport_color_silv_rans_regular_nomodified.txt", "wt");
		}
	}
	for (i = t[0]; i <= t[1]; i++)
	{
