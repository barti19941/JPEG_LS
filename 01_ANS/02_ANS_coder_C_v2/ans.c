...
void tAnsInitialize(unsigned char* data, unsigned short int size, unsigned char step)
{
	...
	tans.s[0] = 0; tans.s[1] = 1; tans.s[2] = 2;
	tans.Ls = (unsigned char*)calloc(tans.max, sizeof(unsigned char));
	tans.Ls[0] = 3; tans.Ls[1] = 8; tans.Ls[2] = 5;
	tans.symbols = (unsigned char*)calloc(tans.L, sizeof(unsigned char));
	tans.symbols[0] = 1; tans.symbols[1] = 1; tans.symbols[2] = 0; tans.symbols[3] = 2;
	tans.symbols[4] = 2; tans.symbols[5] = 1; tans.symbols[6] = 0; tans.symbols[7] = 2;
	tans.symbols[8] = 1; tans.symbols[9] = 0; tans.symbols[10] = 2; tans.symbols[11] = 1;
	tans.symbols[12] = 2; tans.symbols[13] = 1; tans.symbols[14] = 1; tans.symbols[15] = 1;
...
unsigned char* tAnsEncodingBinaryWhole(unsigned char* data, unsigned short int data_size, unsigned short int* bitstream_size)
{
	...
	for (i = 0; i < data_size; i++)
	{
		...
		x = tans.encodingTable[tans.start[data[i]] + (x >> nBits)];
	}
	AppendToBitstream(x, tans.R, bitstream);
	(*bitstream_size) += tans.R;
	return bitstream;
}
