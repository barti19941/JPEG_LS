...
{
	uint8_t* dout_regular;
	uint8_t* dout_run;
	uint32_t i = 0;
	dout_regular = (uint8_t*)calloc(byte_count_regular, sizeof(uint8_t));
	dout_run = (uint8_t*)calloc(byte_count_run, sizeof(uint8_t));
	for (i = 0; i < byte_count_regular; i++)
	{
		dout_regular[i] = enc.bitstream_regular[byte_count_regular - i - 1];
		dout_regular[i]  = (dout_regular[i]  & 0xf0) >> 4 | (dout_regular[i]  & 0x0f) << 4;
   		dout_regular[i]  = (dout_regular[i]  & 0xcc) >> 2 | (dout_regular[i]  & 0x33) << 2;
   		dout_regular[i]  = (dout_regular[i]  & 0xaa) >> 1 | (dout_regular[i]  & 0x55) << 1;
   	}
   	for (i = 0; i < byte_count_run; i++)
   	{
   		dout_run[i] = enc.bitstream_run[byte_count_run - i - 1];
		dout_run[i]  = (dout_run[i]  & 0xf0) >> 4 | (dout_run[i]  & 0x0f) << 4;
   		dout_run[i]  = (dout_run[i]  & 0xcc) >> 2 | (dout_run[i]  & 0x33) << 2;
   		dout_run[i]  = (dout_run[i]  & 0xaa) >> 1 | (dout_run[i]  & 0x55) << 1;
	}
	free(enc.bitstream_regular);
	free(enc.bitstream_run);
	enc.bitstream_regular = dout_regular;
	enc.bitstream_run = dout_run;
} 
...
static void RunModeProcessing()
{
 	...
 	while(enc.Ix == enc.RUNval)
	{
 	...
 	while(enc.RUNcnt >= (1 << enc.J[enc.RUNindex[current_component]]))
	{
    	AppendToBitStreamRun(1, 1);
    	enc.RUNcnt -= (1 << enc.J[enc.RUNindex[current_component]]);
    	if(enc.RUNindex[current_component] < 31)
		{
			enc.RUNindex_prev[current_component] = enc.RUNindex[current_component];
      		enc.RUNindex[current_component]++;
    	}
 	}
 	if(enc.Ra != enc.Ix)
	{
   		AppendToBitStreamRun(0, 1);
   		AppendToBitStreamRun(enc.RUNcnt, enc.J[enc.RUNindex[current_component]]);
   		if(enc.RUNindex[current_component] > 0)
		{
     		enc.RUNindex_prev[current_component] = enc.RUNindex[current_component];
     		enc.RUNindex[current_component] = enc.RUNindex[current_component] - 1;
   		}
  		if(enc.Ra == enc.Rb)
  		{
  			enc.RItype = 1;
...
{	
	...
	memcpy(dout, enc.bitstream_regular, byte_count_regular);		
	memcpy(dout + byte_count_regular, enc.bitstream_run, byte_count_run);
